...
6. Определить самые популярные платформы (3 шт.), жанры (3 шт.).
   ...
   Описание данных
Набор данных предоставляет различную информацию о пациентах, включая возраст, пол, артериальное давление, уровень холестерина, электрокардиографические характеристики (ЭКГ) и многое другое.
Информация о наборе данных.
age: Возраст пациента.
sex: Пол пациента (0: женщина, 1: мужчина).
cp: Тип боли в груди.
trestbps: кровяное давление в состоянии покоя.
chol: Холестерин сыворотки.
...
oldpea: депрессия ST, вызванная нагрузкой по сравнению с отдыхом.
slp: отклонение пикового сегмента ST при нагрузке.
caa: количество крупных сосудов, окрашенных при рентгеноскопии.
thall: тип дефекта.
output: диагностика сердечно-сосудистых заболеваний.

Задание:
1. Изучить общую информацию о данных. (Получить представлении о наборе данных с помощью методов shape, head, describe, info библиотеки pandas. Посчитать количество пустых значений в признаках с помощью библиотеки numpy.)
2. Произвести разведочный анализ данных по данному набору данных – визуализировать данные с помощью библиотек matplotlib, seaborn:
– Парные диаграммы
– Тепловую карту по матрице корреляции
– Гистограммы
– Ящик с усами (диаграмма размаха)
3. Произвести предобработку данных:
– Удалить из набора данных признаки, слабо коррелирующие между собой и с вероятностью инфаркта.
– Закодировать категориальные признаки в дискретные величины с помощью метода get_dummies.

pandas.get_dummies() используется для манипулирования данными, преобразует категориальные данные в вымышленные или индикаторные переменные.
синтаксис:
pandas.get_dummies(data, prefix=None, prefix_sep='_', dummy_na=False, columns=None, sparse=False, drop_first=False, dtype=None)

– Произвести нормализацию данных с помощью метода RobustScaler библиотеки sklearn.


sklearn.preprocessing.RobustScaler() масштабирует объекты, используя статистику, устойчивую к выбросам, центрирование и масштабирование происходят независимо для каждого признака путем вычисления соответствующей статистики по выборкам в обучающем наборе.
...
– Разбить обработанный набор данных на обучающую и тестовую выборки с помощью метода train_test_split библиотеки sklearn.
4. Произвести обучение следующих моделей библиотеки sklearn:
– Метод опорных векторов (Support vector machine)
– Логистическая регрессия (LogisticRegression)
– Дерево решений (Decision Tree)
– Случайный лес (RandomForest)
5. Отобразить корректность работы каждой модели на тестовой и обучающей выборках с помощью метрики accuracy библиотеки sklearn
